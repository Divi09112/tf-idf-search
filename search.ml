		...
		while not (!found) do
			try
				...
				|(did,indexes) -> if did = docid then 
									(Array.append indexes [|wordid|] ; found:= true)
									else iter2:=!iter2 +1 
			with e->
...
let docsearch doc docid=
	let str = ref doc and wordcount = ref 0 in
		let i = ref (spcindex !str) in
			...
			done; invertindex (lowercase !str) docid !wordcount ; docwordcounts:= !docwordcounts@[!wordcount +1] ; totaldocs:= !totaldocs+1
...
let indxtostr indxlist d=
	...
	while !f do
	try
		retstr:= !retstr^(string_of_int (indxlist.(!tot)))^";" ;tot:= !tot+1 
